...
HPRectangle = sprite_add(working_directory+"./Image/MeunButton/10.png",0,0,0,0,0)
MPRectangle = sprite_add(working_directory+"./Image/MeunButton/11.png",0,0,0,0,0)
APRectangle = sprite_add(working_directory+"./Image/MeunButton/12.png",0,0,0,0,0)
...
MeunFile = working_directory + "./菜单布置/强化.ini";
...
if file_exists(MeunFile) {
	...
	#region加载素材和布置按钮show_debug_message("读取" + MeunFile + "开始加载素材")
	...
	for (var i = 0; i < Size; ++i) {
		...
		map[? "Cx"] = CX;
		map[? "Cy"] = CY;
		map[? "图片"] = sp map[? "不使用Tween"] = ini_read_real(i, "不使用Tween", 1)

		var ID = map[? "id"];
		ID.NoUseTween = map[? "不使用Tween"];
		ID.Control = id
...
if true and room = Room[? "主菜单"] // 游戏房间开始有 调整立绘用的
{
	...
	map[? "id"].image_xscale = 0.8 map[? "id"].image_yscale = 0.8
	var ID = map[? "id"]
	ID.NoUseTween = true
