...
if Spr//(curIndex mod 7) == 2   //just some sample stuff to change the variable height by lineitem
    {
      curHeight = sprite_get_height(Spr)+4;     
      //hilite is bigger so we draw a bigger hilite box  
      if curSelected then {
		  //draw_rectangle_color(curX,curY,curX+curWidth,curY+curHeight,c_blue,c_blue,c_navy,c_navy,false);
                           draw_set_color(c_white);
                          } 
      draw_sprite(Spr,0,curX+15, curY+1);
      draw_text(curX+100,curY+5,string_hash_to_newline(curLineSt));
    }
    else 
	
	draw_text(curX+4,curY+2,string_hash_to_newline(curLineSt));
