/// @description  draw_string(x, y, text, [sep, w, max]);
...
//separation set
if (_sep==-1){
    _sep = string_height(string_hash_to_newline(string_copy(_text, 1, 1))) * 1.2;
}
...
for(var i=0; i<min(_max, string_length(_text)); i++){
    ...
    //draw
    if (!nextLine){
        draw_text(_x + _total_w, _y + _total_h, string_hash_to_newline(_word));
    }
    
    //add to drawn width
    var _word_w = string_width(string_hash_to_newline(_word));
    ...
    if (_word==" "){
        ...
        var _ext_len = _total_w + string_width(string_hash_to_newline(_full_word));
        if (_ext_len > _w){
            _total_w = _w + 1;
        }
    }
